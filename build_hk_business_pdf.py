...
def build():
    ...
    pdf.cell(usable_w(pdf), 7, "工业自动化  ·  流程控制贸易  ·  进口替代", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(2)
    ...
    heading(pdf, "来港以后多做的一块")
    para(
        pdf,
        "到香港以后，思路变了。内地在推进口产品替代，以前装置上用的进口件、进口方案，工厂越来越想自己造。我在这条链上做了这些年，知道产品怎么拆、程序怎么写、料从哪配。现在就用这一点，帮内地公司开发他们准备投产的产品：核心程序、设计，再把供应链搭起来。贸易还在做，这块是新长出来的——从把现成的进口货卖进去，转向帮人把进口货变成他们自己能造的东西。",
    )
    para(
        pdf,
        "程序和设计在香港做，合同和收款也可以走香港公司。供应链仍然连着原来的生产厂、代理商和器件渠道，只是目的从「给项目供现货」变成「给要国产化的厂家备料、配型、把关键环节做出来」。",
    )
